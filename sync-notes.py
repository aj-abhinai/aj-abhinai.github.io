...
DEST_DIR = Path(__file__).parent / "content" / "Notes"
...
def has_publish_tag(content: str) -> bool:
    """Check if the first body line (after frontmatter) contains the #published tag."""
    _, body = parse_frontmatter(content)
    first_line = body.split("\n", 1)[0]
    return bool(re.search(r'#published\b', first_line, re.IGNORECASE))
...
def process_assets(content: str, source_file: Path, assets_copied: set[str]) -> tuple[str, int]:
    ...
    def replace_obsidian_embed(match):
        ...
        # Strip alias/size suffix (e.g. ![[image.png|300]]) and any folder path
        asset_name = target.split("|", 1)[0].strip()
        asset_name = asset_name.replace("\\", "/").split("/")[-1].strip()
    ...
    def replace_md_image(match):
        ...
        # Skip external URLs (./ and relative paths are resolved below)
...
def process_embedded_notes(content: str, published_names: set[str]) -> str:
    ...
    def replace_embed(match):
        target = match.group(1).strip()
        # Strip alias suffix (e.g. ![[Note|alias]]) — dropped on output
        if "|" in target:
            target = target.split("|", 1)[0].strip()
        head, sep, tail = target.partition("#")
        # Normalize: strip folder path and .md extension before comparing
        note_name = head.replace("\\", "/").split("/")[-1].strip()
        if note_name.lower().endswith(".md"):
            note_name = note_name[:-3]
        suffix = f"#{tail}" if sep else ""
        if not note_name:
            return match.group(0)
...
def process_backlinks(content: str, published_names: set[str], current_note: str) -> str:
    ...
    def replace_wikilink(match):
        ...
        if "#" in target_part:
            ...
            note_part, anchor_tail = target_part.split("^", 1)
            # Normalize bare block refs ([[Note^abc]]) to [[Note#^abc]] so Quartz resolves them
            anchor = f"#^{anchor_tail}"
...
def remove_published_tag(content: str) -> str:
    """Remove #published tag from the first body line only."""
    lines = content.split('\n')
    if lines:
        lines[0] = re.sub(r'#published\s*', '', lines[0], flags=re.IGNORECASE).rstrip()
        if not lines[0]:
            lines = lines[1:]
    return '\n'.join(lines).strip()
    ...
    for source_file in all_files:
        ...
        try:
            ...
            if dest_file.exists():
                ...
                # Generate or use existing frontmatter, normalized the same way as updates
                if source_fm:
                    fm = update_frontmatter_with_modified(source_fm)
                else:
                    fm = update_frontmatter_with_modified(generate_new_frontmatter(filename))
